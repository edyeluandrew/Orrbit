...
import { Check, CreditCard, X, Loader, Youtube, MessageCircle, Send, FileText, Book, Crown, Star, Users, DollarSign, TrendingUp, TrendingDown, Sparkles, AlertCircle, Split } from 'lucide-react';
...
// Icon mapping for creator-focused service types
const iconMap = {
  // Creator content types
  YouTube: { Icon: Youtube, color: 'from-red-500/20 to-red-600/10', iconColor: 'text-red-400' },
  Video: { Icon: Youtube, color: 'from-red-500/20 to-red-600/10', iconColor: 'text-red-400' },
  Newsletter: { Icon: FileText, color: 'from-blue-500/20 to-blue-600/10', iconColor: 'text-blue-400' },
  Community: { Icon: Users, color: 'from-purple-500/20 to-purple-600/10', iconColor: 'text-purple-400' },
  Discord: { Icon: MessageCircle, color: 'from-indigo-500/20 to-indigo-600/10', iconColor: 'text-indigo-400' },
  Telegram: { Icon: Send, color: 'from-sky-500/20 to-sky-600/10', iconColor: 'text-sky-400' },
  Course: { Icon: Book, color: 'from-emerald-500/20 to-emerald-600/10', iconColor: 'text-emerald-400' },
  Premium: { Icon: Crown, color: 'from-amber-500/20 to-amber-600/10', iconColor: 'text-amber-400' },
  VIP: { Icon: Star, color: 'from-yellow-500/20 to-yellow-600/10', iconColor: 'text-yellow-400' },
  Alpha: { Icon: TrendingUp, color: 'from-green-500/20 to-green-600/10', iconColor: 'text-green-400' },
  ...
  useEffect(() => {
    const loadServices = () => {
      ...
      if (saved) {
        ...
        // Filter out legacy SaaS services (GPT Pro, Netflix, etc.)
        const filteredProviders = providers.filter(p => !LEGACY_SERVICES.includes(p.name));
        
        // If we filtered something out, update localStorage
        if (filteredProviders.length !== providers.length) {
          if (filteredProviders.length > 0) {
            localStorage.setItem('orbit-service-providers', JSON.stringify(filteredProviders));
          } else {
        ...
        const mappedServices = filteredProviders
          .filter(p => p.active)
          .map(p => {
        ...
        // No default services - creators will be added by admins
        setServices([]);
      }
    };
    ...
    <div className="card-neumorphic p-6 sm:p-8 animate-fade-in border-orbit-gold/10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-display font-bold text-white tracking-wider uppercase">Creators</h2>
        </div>
      ...
      {services.length === 0 ? (
        <div className="text-center py-16 px-6">
          <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-orbit-gold/10 border border-orbit-gold/20 flex items-center justify-center">
            <Users size={40} className="text-orbit-gold/60" />
          </div>
          <h3 className="text-xl font-display font-bold text-white mb-2">No Creators Yet</h3>
          <p className="text-orbit-gray max-w-md mx-auto">
            No content creators have been added yet. Check back soon or explore creator profiles to subscribe directly.
          </p>
        </div>
      ) : (
